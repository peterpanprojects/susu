...
export const DeveloperAdminPage: React.FC<DeveloperAdminPageProps> = ({ initialTab = 'overview', onNavigate }) => {
  ...
  const totalPayoutPool = schedule.reduce((sum, s) => sum + s.expectedPoolAmount, 0);
  const platformCommission = totalVolumeCollected * 0.02; // 2% platform fee
  ...
  const filteredAgents = agents.filter((agent) => {
    ...
    if (term) {
      const matchesSearch =
        ...
        (agent.licenseNumber && agent.licenseNumber.toLowerCase().includes(term)) ||
        (agent.kycData?.fullName && agent.kycData.fullName.toLowerCase().includes(term));

      if (!matchesSearch) return false;
    }

    if (agentStatusFilter === 'verified') return agent.adminApprovalStatus === 'verified';
    if (agentStatusFilter === 'pending') return agent.adminApprovalStatus === 'pending_admin_approval' || (agent.isKycSubmitted && agent.adminApprovalStatus !== 'verified');
  ...
  const renderDocumentPreview = (label: string, src?: string) => {
    if (!src) return null;
    return (
      <div key={label} style={{ border: '1px solid #e2e8f0', borderRadius: '10px', overflow: 'hidden', background: '#fff', position: 'relative' }}>
        <div style={{ padding: '0.45rem 0.65rem', fontSize: '0.72rem', fontWeight: 700, color: '#334155', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>{label}</span>
          <button
            type="button"
            onClick={() => setAdminLightboxImg({ label, src })}
            style={{ background: 'none', border: 'none', color: 'var(--color-emerald-700)', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '2px' }}
          >
            Zoom
          </button>
        </div>
        <img
          ...
          alt={label}
          onClick={() => setAdminLightboxImg({ label, src })}
          style={{ display: 'block', width: '100%', height: '150px', objectFit: 'cover', cursor: 'pointer' }}
